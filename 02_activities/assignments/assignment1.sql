...
SELECT 
    ...
    SUM(cp.quantity * cp.cost_to_customer_per_qty) AS price
FROM 
    customer_purchases AS cp
INNER JOIN 
    customer AS c ON cp.customer_id = c.customer_id
GROUP BY 
    cp.customer_id, c.customer_last_name, c.customer_first_name  
HAVING 
...
SELECT *,   strftime('%m', market_date) AS month,
